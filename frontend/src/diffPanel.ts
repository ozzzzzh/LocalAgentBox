...
export class DiffPanel {
   ...
   * 计算 diff 统计
   */
  private computeStats(lines: DiffLine[]): { adds: number; removes: number } {
    return {
   ...
   * 渲染行号
   */
  private renderLineNumbers(lines: DiffLine[], side: "old" | "new"): string {
    return lines
      .map((line) => {
   ...
   * 渲染 diff 内容
   */
  private renderDiffContent(lines: DiffLine[]): string {
    return lines
      .map((line) => {
